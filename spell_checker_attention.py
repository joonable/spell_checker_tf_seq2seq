...
class SpellChecker():
    def __init__(self, config):
        ...
        with tf.variable_scope('decode'):
            ...
            self.decoder = tf.contrib.seq2seq.BasicDecoder(
                ...
                initial_state = self.dec_cell.zero_state(self.current_batch_size, tf.float32).clone(cell_state = self.enc_states),
                output_layer = self.projection_layer)
            self.encoder_inputs.get_shape()
            # self.decoder.zero_state(self.batch_size, tf.float32).clone(cell_state = self.enc_states)
    ...
    def train(self):
        ...
        for train_batch in train_batches:
            ...
            train_enc_input_batch, train_dec_input_batch, train_dec_output_batch, \
            train_target_weights_batch, train_enc_len_batch, train_dec_len_batch, current_batch_size_batch \
            ...
            if current_step % self.n_eval == 0:
                ...
                if train_accuracy > train_best_accuracy and val_accuracy > val_best_accuracy:
                    ...
                    train_best_accuracy, val_best_accuracy, self.best_at_step \
                        = train_accuracy, val_accuracy, current_step
