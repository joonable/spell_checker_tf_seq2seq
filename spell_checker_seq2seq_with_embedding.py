...
class SpellChecker():
    def __init__(self, config):
        ...
        self.training_mode = tf.placeholder(tf.bool, [None, None], name = "training_mode")
        ...
        self.decoder_inputs = tf.placeholder(dtype = tf.float32, shape = [None, None, self.n_input],
                                             name = "decoder_inputs")
        self.decoder_outputs = tf.placeholder(tf.int64, [None, None, self.n_class], name = "decoder_outputs")
        ...
        self.embedding = tf.get_variable("embedding", [self.dic_len, self.embedding_size])
        ...
        self.decoder_emb_outp = tf.nn.embedding_lookup(self.embedding, self.decoder_outputs)
        ...
        with tf.variable_scope('decode'):
            ...
            if self.training_mode == True:
                ...
                self.helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(self.embedding,
        ...
        with tf.variable_scope('cost'):
            self.crossent = tf.nn.softmax_cross_entropy_with_logits(logits = self.logits, labels = self.decoder_emb_outp)
    ...
    def test(self):
        self.saver.restore(self.sess, save_path = self.checkpoint_prefix + '-' + str(self.best_at_step))
